...
class PresentationService {
  ...
  Future<List<AppSlide>> getPresentation(String uuid) async {
    ...
    if (response.statusCode == 200) {
      ...
      _presentationName = presentation.id.name;
      Slide slide;
      int slideIndex = 0;
        ...
        for (var j = 0; j < presentation.groups[i].slides.length; j++) {
          ...
          appSlides.add(
            AppSlide(
              ...
              notes: slide.notes,
              text: slide.text,
              label: j == 0 && presentation.groups[i].name != ''
                  ? '${presentation.groups[i].name} ${slide.label}'
                  : slide.label,
              index: slideIndex,
              groupColor: presentation.groups[i].color!,
            ),
          );
          slideIndex++;
        }
